...
sleep 1
	...
	#read FAKE
#for subdir in \
	for mail_part in $(rsync -nav --delete $mdir_fro/ $mdir_to/ | grep deleting | sed 's/deleting //' | grep '/cur/1'); do
	#ls -ld $mdir_fro/$subdir
	#for mail in $(ls -1 $mdir_fro/$subdir/cur/* | grep '\/cur\/14'); do
		mail=$mdir_to/$mail_part
	# All the 50 or so lines check in the $mdir_to, that's if the rsync-into dir
	# has those Message-ID identifiable duplicates. Now checking if the
	# rsync-from dir actualy needs those duplicates, but 50 lines below, and
	# comment out up to there.
		echo "##############  START OF LOOP ############"
		echo ls -l \$mail
		ls -l $mail
	...
	#	echo \$msg_id: $msg_id
	#	grep -rIl "$msg_id" $mdir_fro/
	#	echo next various comparisons to \$mail
	#	grep -rIl "$msg_id" $mdir_fro/ | grep -v $mail | wc -l
	#	num_mails_921=$(grep -rIl "$msg_id" $mdir_fro/ | grep -v $mail | wc -l) 
	#	if [ "$num_mails_921" != "0" ]; then
	#		if [ "$num_mails_921" == "1" ]; then
	#			echo is unique
	#			other921=$(grep -rIl "$msg_id" $mdir_fro/ | grep -v $mail );
	#			echo ls -l \$other921
	#			ls -l $other921
	...
	#		fi
		...
		echo next various comparisons to \$mail
		grep -rIl "$msg_id" $mdir_fro/ | grep -v $mail | wc -l
		num_mails_007=$(grep -rIl "$msg_id" $mdir_fro/ | grep -v $mail | uniq | wc -l) 
		if [ "$num_mails_007" != "0" ]; then
			if [ "$num_mails_007" == "1" ]; then
				echo is unique
				other007=$(grep -rIl "$msg_id" $mdir_fro/ | grep -v $mail | uniq);
				echo ls -l \$other007
				ls -l $other007
				echo diff -s $mail $other007
				if ( diff -s $mail $other007 ); then
					if [ "$mail" != "$other007" ]; then
						sha256sum $mail
						sha256sum $mail | cut -d' ' -f1
						mail_sum=$(sha256sum $mail | cut -d' ' -f1)
						sha256sum $other007
						sha256sum $other007 | cut -d' ' -f1
						other007_sum=$(sha256sum $other007 | cut -d' ' -f1)
						if [ "$mail_sum" == "$other007_sum" ]; then
							rm -v $mail # First is testing with 'rm -iv', then turn to 'rm -v'
						fi
					fi
				else
					num_ln_diff=$( diff -s $mail $other007 | wc -l )
					echo \$num_ln_diff: $num_ln_diff
					if [ "$num_ln_diff" == 2 ]; then
						if ( diff -s $mail $other007 | grep '> Content-Length: ' ); then
							rm -v $mail # See a few lines above
						fi
					fi
				fi
				...
				echo Redo
				grep -rIl "$msg_id" $mdir_fro/ | grep -v $mail | uniq | wc -l
				#read FAKE
				if [ "$num_mails_007" -gt "1" ]; then
					echo Take only the top line, later trying bottom line
					grep -rIl "$msg_id" $mdir_fro/ | grep -v $mail | uniq | head -1
					other007=$(grep -rIl "$msg_id" $mdir_fro/ | grep -v $mail | uniq | head -1);
					echo ls -l \$other007
					ls -l $other007
					echo diff -s $mail $other007
					if [ -e $mail ]; then
						grep -rIl "$msg_id" $mdir_fro/ | grep -v $mail | uniq
						grep -rIl "$msg_id" $mdir_fro/ | grep -v $mail | uniq | head -1
						other007=$(grep -rIl "$msg_id" $mdir_fro/ | grep -v $mail | uniq | head -1);
						echo ls -l \$other007
						ls -l $other007
					...
					#Now if $mail still exist, try bottom line
					ls -l $mail
					#read FAKE
					if [ -e $mail ]; then
						grep -rIl "$msg_id" $mdir_fro/ | grep -v $mail | uniq
						grep -rIl "$msg_id" $mdir_fro/ | grep -v $mail | uniq | tail -1
						other007=$(grep -rIl "$msg_id" $mdir_fro/ | grep -v $mail | tail -1);
						echo ls -l \$other007
						ls -l $other007
						echo diff -s $mail $other007
						if ( diff -s $mail $other007 ); then
					...
					echo Messed up at this point. Got lost.
				fi
			fi
